...
def get_choco_time(absolute_dataset_log_file):
    cputime = "100000"
    
    with open(absolute_dataset_log_file, 'r') as logfile:
        for line in logfile:
            line_parse = re.findall(r'durée =[\s]+(\d+\.\d+)', line)
            if line_parse:
                cputime = '{0:.2f}'.format(float(line_parse[0]))
                    ...
                    if line_parse[0] == '0':
                        with open(absolute_dataset_ana_file, 'r') as anafile:
                           ...
                           #  cputime = '{0:.2f}'.format(float(resval[-7]))
                            cputime = get_choco_time(absolute_dataset_log_file)
