...
def evaluateQuality_ClosedDiversity(queue, script_file_name, project_res_dir, config):
    try:
        # preparing directories
...
if __name__ == '__main__':
        ...
        if filename.endswith(".dat"):
            if filename in fthresholds:
                for threshold in fthresholds[filename]:
                    for div in diversity_thresholds:
    ...
    for i in range(0, npr):
        if curr < len(configs_diversity):
            # launch
            print(configs_diversity[i][0], configs_diversity[curr])
            proc = Process(target=evaluateQuality_ClosedDiversity, args=(queue, scriptFileName, project_res_dir, configs_diversity[curr],))
            proc.start()
