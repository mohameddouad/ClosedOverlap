...
# program parameters
methods = ["ClosedDiversity_JaccardChecker"]
                ...
                if line_parse:
                    if line_parse[0] == '0':
                        with open(absolute_dataset_ana_file, 'r') as anafile:
                            first_line = anafile.readline()
                            resval = first_line.split(';')
                            cputime = '{0:.3f}'.format(float(resval[-7]))
                            nbr_pattern = resval[-6]
                            nbr_pattern_witness = resval[-4]
                            nbr_nodes = resval[-5]
                            nbr_var_filtred_jac = resval[-3]
                            nbr_fails = resval[-1]
                            nbr_fails_jac = resval[-2]
                    elif line_parse[0] == '124':
                        cputime = "-"
                        nbr_pattern = "-"
                        nbr_nodes = "-"
                        ...
                        nbr_fails = "-"
                        nbr_fails_jac = "-"
    return cputime, nbr_pattern, nbr_nodes, nbr_pattern_witness, nbr_var_filtred_jac, nbr_fails, nbr_fails_jac
...
def parseClosedDiversity_JaccardChecker(head_param, content_param):
    '''
    ...
    if head_param:
        head_param[0] += 5
        return ['|c|c|c|c|c', '& \\multicolumn{5}{c|} {Closed Diversity Jaccard-Checker} ', '& Jmax & CPU-Time & \\#Patterns (\\#Witness) & \\#Nodes (\\#Fil. Var.) & \\#Fails (\\#Fails by Jac.)'], head_param[0];
    elif content_param:
        data_name = content_param[0]
        absolute_dataset_res_dir = content_param[1]
        ...
        absolute_dataset_log_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, data_name + "-" + content_param[2].replace(".", "v") + "-" + content_param[3].replace(".", "v") + "-" + content_param[4] + "-" + content_param[6] + "-" + content_param[7] + ".log"))
        cputime, nbr_pattern, nbr_nodes, nbr_pattern_witness, nbr_var_filtred_jac, nbr_fails, nbr_fails_jac = readResultClosedDiversity(absolute_dataset_res_file, absolute_dataset_ana_file, absolute_dataset_log_file)
        return ' & ' + content_param[3] + ' & ' + cputime + ' & ' + nbr_pattern + ' (' + nbr_pattern_witness + ')' + ' & ' + nbr_nodes + ' (' + nbr_var_filtred_jac + ')' + ' & ' + nbr_fails + ' (' + nbr_fails_jac + ')'
