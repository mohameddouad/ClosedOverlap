...
def closedDiversity(queue, project_bin_file, project_data_dir, project_res_dir, config):
    ...
    try:
        ...
        absolute_dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, "ClosedDiversity_1_estFreq_WITNESS", config[0].split('.')[0]))
...
if __name__ == '__main__':
    ...
    for i in range(0, npr):
        if curr < len(configs):
            ...
            proc = Process(target=closedDiversity, args=(queue, project_bin_closed_diversity_1_estFreq_file, project_data_dir, project_res_dir, configs[curr],))
            proc.start()
