...
def closedDiversityJaccardGraph(queue, script_file_name, project_res_dir, config):
    ...
    try:
        absolute_dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, "ClosedDiversityNew_estFreq", 
                                                                 config[0].split('.')[0]))
        
        absolute_dataset_images_dir = os.path.abspath(os.path.join(project_dir, "images", "ClosedDiversityNew_estFreq", 
                                                                 config[0].split('.')[0]))
        ...
        absolute_dataset_image_file = os.path.abspath(os.path.join(
                absolute_dataset_images_dir, config[0].split('.')[0] + "-" + config[1].replace(".", "v") + 
                "-" + config[3].replace(".", "v") + "-" + aggreg + "-" + config[4] + "-0-" + timeout + ".png"))
        ...
        subprocess.run(["/usr/bin/timeout " + timeout + " python3 '" + absolute_script_file + 
                        "' '" + absolute_dataset_eval_file + "' '" + absolute_dataset_image_file + 
                        "' " + config[0].split('.')[0] + " " + config[1] + " " + config[3] + "; echo \"Exit status: $?\" "], 
                        shell=True, check=True, stdout=launch_log_file, stderr=launch_log_file)
    finally:
...
if __name__ == "__main__":
    ...
    procs = dict()
    queue = mp.Queue()
    scriptFileName = "graphe_jaccard_v2.py"
