...
def closedDiversityJaccardGraph(queue, script_file_name, project_res_dir, config):
    ...
    try:
        absolute_dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, "foxrex", "ClosedDiversityNew", 
                                                                 config[0].split('.')[0]))
        
        absolute_dataset_images_dir = os.path.abspath(os.path.join("images", "foxrex", "ClosedDiversityNew", 
                                                                 config[0].split('.')[0]))
        ...
        subprocess.run(["/usr/bin/timeout " + timeout + " python3 '" + absolute_script_file + 
                        ...
                        "' " + config[1] + " " + config[3] + "; echo \"Exit status: $?\" "], 
...
if __name__ == "__main__":
    ...
    procs = dict()
    queue = mp.Queue()
    scriptFileName = "graphe_jaccard.py"
