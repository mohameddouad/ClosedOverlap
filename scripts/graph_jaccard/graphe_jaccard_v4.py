...
if __name__ == "__main__":
    ...
    for div in range(0, len(diversitythresholds)):
        ...
        for strat in range(0, len(searchstrategies)):
            debut_X = 0
            subres_dir = "ClosedDiversityJMAXVar_"
            if searchstrategies[strat] == "MINCOV":
                subres_dir = subres_dir+searchstrategies[strat]+"_V2_estFreq"
            else:
                subres_dir = subres_dir+"NewUB_"+searchstrategies[strat]+"_estFreq"
            
            dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, subres_dir, dataset_name))
            ...
            plt.subplot(1, len(searchstrategies), it, title=titre)
            plt.scatter(x, y_lb, s=20, c='blue', marker='o', label='LB')
            plt.scatter(x, y_jac, s=20, c='green', marker=(5, 2), label='Jaccard')
            plt.scatter(x, y_ub, s=20, c='orange', marker='+', label='UB')
            plt.scatter(x, y_jmax, s=20, c='red', marker='_', label='Jmax')
        ...
        imageFile = os.path.abspath(os.path.join(dataset_images_dir, dataset_name + 
                "-" + fmin.replace(".", "v") + "-" + diversitythresholds[div].replace(".", "v") + 
                "-" + aggreg + "-0-" + timeout + ".png"))
        
        plt.savefig(imageFile)
        
        print("\n********** Picture saved for Jmax =", str(100*float(diversitythresholds[div]))+"%", "**********\n")
