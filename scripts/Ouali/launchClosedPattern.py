...
def closedPattern(queue, project_bin_file, project_data_dir, project_res_dir, config):
    ...
    try:
        ...
        absolute_dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, "foxrex", "Closed", config[0].split('.')[0]))
        ...
        subprocess.run(["/usr/bin/timeout " + config[3] + " java -jar " + project_bin_file + "  '" + absolute_dataset_file + "' " + config[2] + " '" + absolute_dataset_res_file + "' '" + absolute_dataset_ana_file + "'; echo \"Exit status: $?\" "], shell=True, check=True, stdout=launch_log_file, stderr=launch_log_file)
    finally:
