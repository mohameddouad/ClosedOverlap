...
def closedDiversity(queue, project_bin_file, project_data_dir, project_res_dir, config):
    ...
    try:
        ...
        absolute_dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, "ClosedDiversity_3_estFreq_WITNESS", config[0].split('.')[0]))
        os.makedirs(absolute_dataset_res_dir, exist_ok=True)
        absolute_dataset_res_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1].replace(".", "v") + "-" + config[3].replace(".", "v") + "-" + config[4] + "-" + config[5] + "-" + witnessStrategy + "-" + config[6] + "-" + config[7] + ".res"))
        absolute_dataset_ana_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1].replace(".", "v") + "-" + config[3].replace(".", "v") + "-" + config[4] + "-" + config[5] + "-" + witnessStrategy + "-" + config[6] + "-" + config[7] + ".ana"))
        absolute_dataset_log_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1].replace(".", "v") + "-" + config[3].replace(".", "v") + "-" + config[4] + "-" + config[5] + "-" + witnessStrategy + "-" + config[6] + "-" + config[7] + ".log"))
        ...
        print("/usr/bin/timeout " + config[7] + " java -jar " + project_bin_file + "  '" + absolute_dataset_file + "' '" + absolute_dataset_res_file + "' '" + absolute_dataset_ana_file + "' -f " + config[2] + " -j " + config[3] + " -ag " + config[4] + " -s " + config[5] + " " + witnessStrategy + " -th " + config[6])
        subprocess.run(["/usr/bin/timeout " + config[7] + " java -jar " + project_bin_file + "  '" + absolute_dataset_file + "' '" + absolute_dataset_res_file + "' '" + absolute_dataset_ana_file + "' -f " + config[2] + " -j " + config[3] + " -ag " + config[4] + " -s " + config[5] + " " + witnessStrategy + " -th " + config[6] + "; echo \"Exit status: $?\" "], shell=True, check=True, stdout=launch_log_file, stderr=launch_log_file)
    finally:
...
if __name__ == '__main__':
    ...
    for i in range(0, npr):
        if curr < len(configs):
            ...
            proc = Process(target=closedDiversity, args=(queue, project_bin_closed_diversity_3_estFreq_file, project_data_dir, project_res_dir, configs[curr],))
            proc.start()
