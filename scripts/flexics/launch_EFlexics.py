...
#~~ non parsé              "mushroom.fimi" : ["0.05", "0.01", "0.008", "0.005"],
...
# qualityMeasure = ["frequency", "uniform", "purity"]
...
def flexics(queue, project_flexics_script_file, project_data_dir, project_res_dir, config):
    ...
    try:
        ...
        absolute_dataset_res_dir = os.path.abspath(os.path.join(project_res_dir, "Flexics", "EFlexics", config[0].split('.')[0]))
        os.makedirs(absolute_dataset_res_dir, exist_ok=True)
        
        absolute_dataset_temp_res_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1] + "-" + config[2] + "-" + config[4].replace(".", "v") + "-" + config[5] + "-" + config[6].replace(".", "v") + "-" + config[7] + "-" + config[8] + ".txt"))
        
        absolute_dataset_res_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1] + "-" + config[2] + "-" + config[4].replace(".", "v") + "-" + config[5] + "-" + config[6].replace(".", "v") + "-" + config[7] + "-" + config[8] + ".res"))
        
        absolute_dataset_log_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1] + "-" + config[2] + "-" + config[4].replace(".", "v") + "-" + config[5] + "-" + config[6].replace(".", "v") + "-" + config[7] + "-" + config[8] + ".log"))
        
        absolute_time_log_file = os.path.abspath(os.path.join(absolute_dataset_res_dir, config[0].split('.')[0] + "-" + config[1] + "-" + config[2] + "-" + config[4].replace(".", "v") + "-" + config[5] + "-" + config[6].replace(".", "v") + "-" + config[7] + "-" + config[8] + ".time"))
        ...
        if not os.path.exists(project_flexics_script_file):
            ...
            print("Script file location (given in argument) : ", project_flexics_script_file, "\n")
            sys.exit(1)
        ...
        if filename.endswith(".cp4im") or filename.endswith(".fimi"):
            if filename in fthresholds:
                ...
                for i in range(0, len(fthresholds[filename])):
                    ...
                    for quality in qualityMeasure:
                        for kappa in error_tolerance:
                            configs.append((filename, taskTODO, oracle, str(absolute_threshold), fthresholds[filename][i], quality, kappa, allSamples[filename][i], str(timeout)))
