...
def parseClosedDiversityNew(head_param, content_param):
    '''
...
if __name__ == '__main__':
    ...
    try:
        if len(sys.argv) < 3:
            print("ERROR, some parameters miss\n")
            sys.exit(1)
        else:
            jmax = sys.argv[1]
            j = float(jmax)
            branchStrategy = sys.argv[2]
        ...
        print("ERROR : donner une valeur réelle pour jmax puis donner la stratédie de branchement\n")
        sys.exit(1)
